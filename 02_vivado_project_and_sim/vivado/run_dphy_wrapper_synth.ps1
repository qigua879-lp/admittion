param(
    ...
    [string]$ProjectName = "mipi_csi2_capture_dphy_wrapper_synth",
...
try {
    ...
    & $VivadoCmd -mode batch `
        -source (Join-Path $scriptDir "run_synth_only.tcl") `
        -tclargs $ProjectName $TopModule $Jobs

    if ($LASTEXITCODE -ne 0) {
        throw "Vivado D-PHY wrapper synthesis failed."
    }
    ...
    Write-Host "INFO: D-PHY wrapper synthesis completed. Artifacts are under $projectReportRoot"
} finally {
