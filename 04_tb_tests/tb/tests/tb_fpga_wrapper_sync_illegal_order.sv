...
module tb_fpga_wrapper_sync_illegal_order;
    ...
    task automatic drive_illegal_sync_sequence;
        begin
    ...
    always_ff @(posedge clk_sys) begin
        if (!rst_n) begin
            ...
            sync_count_at_end <= dut.u_mipi_csi2_capture_top.err_cnt_sync;
        end
    end
    ...
    initial begin
        ...
        drive_illegal_sync_sequence();

        fork
            begin : timeout_block
                repeat (3000) @(posedge clk_sys);
                $display("FAIL: sync illegal-order timeout cfg_done=%0b sensor_done=%0b frame=%0b line=%0b sync=%0b sync_cnt=%0d",
                         cfg_init_done_o, sensor_done_seen, frame_start_seen, line_start_seen,
                         sync_seen, sync_count_at_end);
                $fatal(1);
            end
            ...
            begin : main_check_block
                ...
                if (!frame_start_seen || !line_start_seen) begin
                    $display("FAIL: expected FS/LS markers not observed frame=%0b line=%0b",
                             frame_start_seen, line_start_seen);
                    $fatal(1);
                end

                if (!sync_seen) begin
                    $display("FAIL: sync error pulse was not observed");
                    $fatal(1);
                end

                if (sync_count_at_end == 32'd0) begin
                    $display("FAIL: sync error count did not increment");
                    $fatal(1);
                end

                if (err_ecc_o || err_crc_o) begin
                    $display("FAIL: unexpected non-sync errors ecc=%0b crc=%0b",
                             err_ecc_o, err_crc_o);
                    $fatal(1);
                end

                if (pixel_valid_o || pixel_sof_o || pixel_sol_o) begin
                    $display("FAIL: illegal short-packet sequence should not produce pixel activity");
                    $fatal(1);
                end

                $display("PASS: tb_fpga_wrapper_sync_illegal_order sync_cnt=%0d",
                         sync_count_at_end);
            end
        join_any
