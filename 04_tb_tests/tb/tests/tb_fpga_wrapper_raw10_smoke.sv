...
module tb_fpga_wrapper_raw10_smoke;
        ...
        begin
            ...
            while (!sensor_lane_ready[tx_lane_phase]) begin
                ...
                if (wait_cycles > 256) begin
                    $display("FAIL: raw10 smoke lane ready timeout phase=%0d ready0=%0b ready1=%0b byte0=0x%02h",
    ...
    task automatic drive_raw10_frame;
        begin
            ...
            // Flush the final FE ECC byte through the current 2-lane wrapper
            // path, which otherwise leaves the trailing single byte buffered.
    ...
    initial begin
        ...
        fork
            begin : cfg_timeout_block
                ...
                if (!cfg_init_done_o) begin
                    $display("FAIL: raw10 smoke boot timeout cfg_done=%0b", cfg_init_done_o);
                    $fatal(1);
                end
            end
        ...
        fork
            begin : timeout_block
                ...
                $display("FAIL: raw10 smoke timeout sensor_done=%0b frame=%0b/%0b line=%0b/%0b exp=%0d act=%0d mismatch=%0d ecc=%0b crc=%0b sync=%0b",
                         sensor_done_seen, frame_start_seen, frame_end_seen, line_start_seen, line_end_seen,
            ...
            begin : main_check_block
                ...
                if (!pass || fail) begin
                    $display("FAIL: raw10 scoreboard pass=%0b fail=%0b exp=%0d act=%0d mismatch=%0d frames=%0d",
                ...
                if (!frame_start_seen || !frame_end_seen || !line_start_seen || !line_end_seen) begin
                    $display("FAIL: missing raw10 markers fs=%0b fe=%0b ls=%0b le=%0b",
                             frame_start_seen, frame_end_seen, line_start_seen, line_end_seen);
                    $fatal(1);
                end

                if (!pixel_sof_seen || !pixel_sol_seen) begin
                    $display("FAIL: missing raw10 pixel markers sof=%0b sol=%0b",
                             pixel_sof_seen, pixel_sol_seen);
                    $fatal(1);
                end

                if (err_ecc_o || err_crc_o || err_sync_o) begin
                    $display("FAIL: unexpected raw10 errors ecc=%0b crc=%0b sync=%0b",
                             err_ecc_o, err_crc_o, err_sync_o);
                    $fatal(1);
                end

                $display("PASS: tb_fpga_wrapper_raw10_smoke exp=%0d act=%0d frames=%0d full_frame=1",
