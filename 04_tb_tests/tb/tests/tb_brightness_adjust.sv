...
module tb_brightness_adjust;
    ...
    task automatic expect_pixel(input logic [23:0] data, input logic sof, input logic sol);
        begin
            ...
            if (pixel_data_o !== data || pixel_sof_o !== sof || pixel_sol_o !== sol) begin
                fail($sformatf("brightness mismatch exp=%06h/%0b/%0b got=%06h/%0b/%0b",
    ...
    initial begin
        ...
        cfg_gain_i = 8'h80;
        cfg_bias_i = 9'sd10;
        send_pixel(24'h102030, 1'b1, 1'b1);
        expect_pixel(24'h1a2a3a, 1'b1, 1'b1);

        cfg_bias_i = 9'sd20;
        send_pixel(24'hf0f8ff, 1'b0, 1'b0);
        expect_pixel(24'hffffff, 1'b0, 1'b0);

        cfg_bias_i = -9'sd20;
        send_pixel(24'h050a0f, 1'b0, 1'b1);
        expect_pixel(24'h000000, 1'b0, 1'b1);

        bypass_i   = 1'b1;
        cfg_gain_i = 8'hff;
        cfg_bias_i = 9'sd100;
        send_pixel(24'h123456, 1'b0, 1'b0);
        expect_pixel(24'h123456, 1'b0, 1'b0);
        ...
        if (!pixel_valid_o || pixel_data_o !== 24'h020304 || !pixel_sof_o || pixel_sol_o || pixel_ready_o) begin
            fail("brightness backpressure hold failed");
        end
        pixel_ready_i = 1'b1;
        @(posedge clk_sys);

        $display("[%0t] PASS: tb_brightness_adjust", $time);
        $finish;
    end
