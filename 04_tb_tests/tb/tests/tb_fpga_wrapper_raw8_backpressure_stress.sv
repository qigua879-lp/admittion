...
module tb_fpga_wrapper_raw8_backpressure_stress;
    ...
    always_ff @(posedge clk_sys) begin
        if (!rst_n) begin
            ...
            if (pixel_valid_o && !dut.u_mipi_csi2_capture_top.final_pixel_ready) begin
                pixel_stall_seen   <= 1'b1;
                pixel_stall_cycles <= pixel_stall_cycles + 1;
            end
    ...
    initial begin
        ...
        fork
            begin : timeout_block
                ...
                $display("FAIL: raw8 backpressure stress timeout cfg=%0b fs=%0b fe=%0b awstall=%0b wstall=%0b pixel_stall=%0b lane_bp=%0b exp=%0d act=%0d mismatch=%0d",
                         cfg_enable_seen, frame_start_seen, frame_end_seen,
                         aw_stall_seen, w_stall_seen, pixel_stall_seen, lane_bp_seen,
            ...
            begin : main_check_block
                ...
                if (!pass || fail) begin
                    $display("FAIL: raw8 backpressure stress scoreboard pass=%0b fail=%0b exp=%0d act=%0d mismatch=%0d",
                             pass, fail, exp_pixel_cnt, act_pixel_cnt, mismatch_cnt);
                    $fatal(1);
                end

                if (!cfg_enable_seen || !frame_start_seen || !frame_end_seen ||
                    !aw_stall_seen || !w_stall_seen) begin
                    $display("FAIL: raw8 backpressure stress missing observations cfg=%0b fs=%0b fe=%0b awstall=%0b wstall=%0b pixel_stall=%0b lane_bp=%0b",
                             cfg_enable_seen, frame_start_seen, frame_end_seen,
                             aw_stall_seen, w_stall_seen, pixel_stall_seen, lane_bp_seen);
                    $fatal(1);
                end
                ...
                if (frame_start_count != FRAME_COUNT || frame_end_count != FRAME_COUNT ||
                    ...
                    $display("FAIL: raw8 backpressure stress counters fs=%0d fe=%0d ls=%0d le=%0d psof=%0d psol=%0d",
                ...
                if (err_ecc_o || err_crc_o || err_sync_o) begin
                    $display("FAIL: unexpected protocol errors during raw8 backpressure stress");
                    $fatal(1);
                end

                $display("RESULT: byte_fifo_aw=%0d axi_fifo_aw=%0d stall=%0d frames=%0d lines=%0d exp=%0d act=%0d aw_bursts=%0d w_beats=%0d aw_stall_cycles=%0d w_stall_cycles=%0d pixel_stall_seen=%0b pixel_stall_cycles=%0d lane_bp_seen=%0b lane_bp_cycles=%0d max_byte_fifo=%0d max_axi_fifo=%0d",
