...
module tb_fpga_wrapper_rgb888_metrics;
    ...
    initial begin
        ...
        fork
            begin : timeout_block
                ...
                $display("FAIL: rgb888 metrics timeout init=%0b frame=%0b first_pixel=%0b frame_end=%0b exp=%0d act=%0d mismatch=%0d",
            ...
            begin : main_check_block
                ...
                if (!pass || fail) begin
                    $display("FAIL: rgb888 metrics scoreboard pass=%0b fail=%0b exp=%0d act=%0d mismatch=%0d",
                ...
                if (!init_done_seen || !frame_start_seen || !first_pixel_seen || !frame_end_seen) begin
                    $display("FAIL: rgb888 metrics missing markers init=%0b frame=%0b first_pixel=%0b frame_end=%0b",
                             init_done_seen, frame_start_seen, first_pixel_seen, frame_end_seen);
                    $fatal(1);
                end

                if (err_ecc_o || err_crc_o || err_sync_o) begin
                    $display("FAIL: unexpected rgb888 protocol errors");
                    $fatal(1);
                end

                $display("PASS: tb_fpga_wrapper_rgb888_metrics init_to_frame=%0d frame_to_first_pixel=%0d frame_to_end=%0d first_to_last_pixel=%0d pixel_valid_cycles=%0d exp=%0d act=%0d",
