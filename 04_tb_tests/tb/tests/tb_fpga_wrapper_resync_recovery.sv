...
module tb_fpga_wrapper_resync_recovery;
    ...
    always_ff @(posedge clk_sys) begin
        if (!rst_n) begin
            ...
            if (dut.u_mipi_csi2_capture_top.axi_clear_busy) begin
                axi_clear_busy_seen <= 1'b1;
            end
        end
    end
    ...
    initial begin
        ...
        fork
            begin : timeout_block
                ...
                $display("FAIL: resync signal-chain timeout sync=%0b req=%0b busy=%0b done=%0b clear=%0b axi_clear=%0b",
                         sync_seen, resync_req_seen, resync_busy_seen, resync_done_seen,
                         resync_clear_seen, axi_clear_busy_seen);
                $fatal(1);
            end
            ...
            begin : main_check_block
                ...
                if (!sync_seen || !resync_req_seen || !resync_busy_seen || !resync_done_seen || !resync_clear_seen) begin
                    $display("FAIL: resync signals were not fully observed sync=%0b req=%0b busy=%0b done=%0b clear=%0b",
                             sync_seen, resync_req_seen, resync_busy_seen, resync_done_seen, resync_clear_seen);
                    $fatal(1);
                end

                if (err_ecc_o || err_crc_o) begin
                    $display("FAIL: unexpected non-sync errors during resync signal-chain test");
                    $fatal(1);
                end

                $display("PASS: tb_fpga_wrapper_resync_recovery sync=%0b req=%0b busy=%0b done=%0b clear=%0b axi_clear=%0b",
                         sync_seen, resync_req_seen, resync_busy_seen, resync_done_seen,
                         resync_clear_seen, axi_clear_busy_seen);
            end
        join_any
