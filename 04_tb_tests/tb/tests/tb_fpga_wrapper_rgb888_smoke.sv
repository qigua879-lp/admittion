...
module tb_fpga_wrapper_rgb888_smoke;

    localparam int LANE_NUM = 2;
    localparam logic [5:0] DATA_TYPE = csi2_reference_helpers_pkg::CSI2_DT_RGB888;
    ...
    task automatic send_long_packet;
        ...
        begin
            ...
            for (idx = 0; idx < csi2_reference_helpers_pkg::csi2_payload_byte_count(DATA_TYPE); idx = idx + 2) begin
                push_lane_group(
                    csi2_reference_helpers_pkg::csi2_payload_byte(DATA_TYPE, idx),
                    csi2_reference_helpers_pkg::csi2_payload_byte(DATA_TYPE, idx + 1)
                );
            end
    ...
    task automatic drive_rgb888_frame;
        begin
    ...
    initial begin
        ...
        force dut.u_mipi_csi2_capture_top.u_cfg_reg_if_apb.cfg_dt_code_o = 8'h24;
        ...
        fork
            begin : timeout_block
                ...
                $display("FAIL: rgb888 smoke timeout frame=%0b/%0b line=%0b/%0b exp=%0d act=%0d mismatch=%0d ecc=%0b crc=%0b sync=%0b",
            ...
            begin : main_check_block
                ...
                if (!pass || fail) begin
                    $display("FAIL: rgb888 scoreboard pass=%0b fail=%0b exp=%0d act=%0d mismatch=%0d frames=%0d",
                ...
                if (!frame_start_seen || !frame_end_seen || !line_start_seen || !line_end_seen) begin
                    $display("FAIL: missing rgb888 markers fs=%0b fe=%0b ls=%0b le=%0b",
                             frame_start_seen, frame_end_seen, line_start_seen, line_end_seen);
                    $fatal(1);
                end

                if (!pixel_sof_seen || !pixel_sol_seen) begin
                    $display("FAIL: missing rgb888 pixel markers sof=%0b sol=%0b",
                             pixel_sof_seen, pixel_sol_seen);
                    $fatal(1);
                end

                if (err_ecc_o || err_crc_o || err_sync_o) begin
                    $display("FAIL: unexpected rgb888 errors ecc=%0b crc=%0b sync=%0b",
                             err_ecc_o, err_crc_o, err_sync_o);
                    $fatal(1);
                end

                $display("PASS: tb_fpga_wrapper_rgb888_smoke exp=%0d act=%0d frames=%0d",
