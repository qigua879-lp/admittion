...
module tb_fpga_wrapper_axi_backpressure;
    ...
    always_ff @(posedge clk_sys) begin
        if (!rst_n) begin
            ...
            if (dut.u_mipi_csi2_capture_top.u_pixel_to_axi_writer.pixel_fire_sys) begin
                pixel_fire_seen <= 1'b1;
            end
        end
    end
    ...
    always_ff @(posedge clk_axi) begin
        if (!rst_n) begin
            ...
            if (dut.u_mipi_csi2_capture_top.u_pixel_to_axi_writer.sys_cmd_pending_q) begin
                sys_cmd_seen <= 1'b1;
            end
            if (dut.u_mipi_csi2_capture_top.u_pixel_to_axi_writer.cmd_fifo_rd_valid) begin
                cmd_fifo_seen <= 1'b1;
            end
            if (dut.u_mipi_csi2_capture_top.u_pixel_to_axi_writer.axi_cmd_valid) begin
                axi_cmd_seen <= 1'b1;
            end
            if (dut.u_mipi_csi2_capture_top.m_axi_awvalid_o) begin
                aw_valid_seen <= 1'b1;
            end
            if (dut.u_mipi_csi2_capture_top.m_axi_wvalid_o) begin
                w_valid_seen <= 1'b1;
            end
    ...
    initial begin
        ...
        fork
            begin : timeout_block
                ...
                $display("FAIL: axi backpressure timeout sensor_done=%0b line_end=%0b cfg_en=%0b pixel_fire=%0b exp=%0d act=%0d mismatch=%0d busy=%0b sys_cmd=%0b cmd_fifo=%0b axi_cmd=%0b awvalid=%0b wvalid=%0b awstall=%0b wstall=%0b",
                         sensor_done_seen, line_end_seen, cfg_enable_seen, pixel_fire_seen,
                         exp_pixel_cnt, act_pixel_cnt, mismatch_cnt,
                         busy_seen, sys_cmd_seen, cmd_fifo_seen, axi_cmd_seen,
                         aw_valid_seen, w_valid_seen, aw_stall_seen, w_stall_seen);
                $fatal(1);
            end
            ...
            begin : main_check_block
                ...
                if (!pass || fail) begin
                    $display("FAIL: axi backpressure scoreboard pass=%0b fail=%0b exp=%0d act=%0d mismatch=%0d",
                             pass, fail, exp_pixel_cnt, act_pixel_cnt, mismatch_cnt);
                    $fatal(1);
                end

                if (!busy_seen || !aw_stall_seen || !w_stall_seen) begin
                    $display("FAIL: expected AXI stall was not fully observed line_end=%0b cfg_en=%0b pixel_fire=%0b busy=%0b sys_cmd=%0b cmd_fifo=%0b axi_cmd=%0b awvalid=%0b wvalid=%0b awstall=%0b wstall=%0b",
                             line_end_seen, cfg_enable_seen, pixel_fire_seen,
                             busy_seen, sys_cmd_seen, cmd_fifo_seen, axi_cmd_seen,
                             aw_valid_seen, w_valid_seen, aw_stall_seen, w_stall_seen);
                    $fatal(1);
                end
                ...
                if (err_ecc_o || err_crc_o || err_sync_o) begin
                    $display("FAIL: unexpected protocol errors under AXI backpressure");
                    $fatal(1);
                end

                $display("PASS: tb_fpga_wrapper_axi_backpressure exp=%0d act=%0d", exp_pixel_cnt, act_pixel_cnt);
            end
        join_any
