...
module tb_fpga_wrapper_yuv422_smoke;

    localparam int LANE_NUM = 2;
    localparam logic [5:0] DATA_TYPE = 6'h1e;
    ...
    function automatic logic [7:0] yuv422_payload_byte(input int unsigned idx);
        begin
            case (idx)
                0: yuv422_payload_byte = 8'h80;
                1: yuv422_payload_byte = 8'h10;
                2: yuv422_payload_byte = 8'h90;
                3: yuv422_payload_byte = 8'h20;
                4: yuv422_payload_byte = 8'h81;
                5: yuv422_payload_byte = 8'h11;
                6: yuv422_payload_byte = 8'h91;
                7: yuv422_payload_byte = 8'h21;
                default: yuv422_payload_byte = 8'h00;
            endcase
        end
    endfunction

    function automatic logic [23:0] yuv422_expected_pixel(input int unsigned idx);
        begin
            case (idx)
                0: yuv422_expected_pixel = 24'h108090;
                1: yuv422_expected_pixel = 24'h208090;
                2: yuv422_expected_pixel = 24'h118191;
                3: yuv422_expected_pixel = 24'h218191;
                default: yuv422_expected_pixel = 24'h000000;
            endcase
        end
    endfunction

    function automatic logic [15:0] yuv422_payload_crc;
        logic [15:0] crc;
        int unsigned idx;
        begin
            crc = 16'hffff;
            for (idx = 0; idx < 8; idx = idx + 1) begin
                crc = csi2_reference_helpers_pkg::csi2_crc16_next_byte(crc, yuv422_payload_byte(idx));
            end
            yuv422_payload_crc = crc;
        end
    endfunction
    ...
    task automatic send_long_packet;
        ...
        int unsigned idx;
        begin
            header      = csi2_reference_helpers_pkg::csi2_pack_header(VC_ID, DATA_TYPE, 16'd8, 1'b0);
            payload_crc = yuv422_payload_crc();
            ...
            for (idx = 0; idx < 8; idx = idx + 2) begin
                push_lane_group(yuv422_payload_byte(idx), yuv422_payload_byte(idx + 1));
            end
    ...
    task automatic feed_expected_pixels;
        ...
        begin
            for (idx = 0; idx < PIXEL_COUNT; idx = idx + 1) begin
                @(negedge clk_sys);
                exp_valid      = 1'b1;
                exp_pixel_data = yuv422_expected_pixel(idx);
                exp_pixel_sof  = (idx == 0);
                exp_pixel_sol  = (idx == 0);
                do begin
                    @(posedge clk_sys);
    ...
    task automatic drive_yuv422_frame;
        begin
    ...
    initial begin
        ...
        force dut.u_mipi_csi2_capture_top.u_cfg_reg_if_apb.cfg_dt_code_o = 8'h1e;
        ...
        fork
            begin : timeout_block
                ...
                $display("FAIL: yuv422 smoke timeout frame=%0b/%0b line=%0b/%0b exp=%0d act=%0d mismatch=%0d ecc=%0b crc=%0b sync=%0b",
                         frame_start_seen, frame_end_seen, line_start_seen, line_end_seen,
                         exp_pixel_cnt, act_pixel_cnt, mismatch_cnt, err_ecc_o, err_crc_o, err_sync_o);
                $fatal(1);
            end
            ...
            begin : main_check_block
                ...
                if (!pass || fail) begin
                    $display("FAIL: yuv422 scoreboard pass=%0b fail=%0b exp=%0d act=%0d mismatch=%0d frames=%0d",
                ...
                if (!frame_start_seen || !frame_end_seen || !line_start_seen || !line_end_seen) begin
                    $display("FAIL: missing yuv422 markers fs=%0b fe=%0b ls=%0b le=%0b",
                             frame_start_seen, frame_end_seen, line_start_seen, line_end_seen);
                    $fatal(1);
                end

                if (!pixel_sof_seen || !pixel_sol_seen) begin
                    $display("FAIL: missing yuv422 pixel markers sof=%0b sol=%0b",
                             pixel_sof_seen, pixel_sol_seen);
                    $fatal(1);
                end

                if (err_ecc_o || err_crc_o || err_sync_o) begin
                    $display("FAIL: unexpected yuv422 errors ecc=%0b crc=%0b sync=%0b",
                             err_ecc_o, err_crc_o, err_sync_o);
                    $fatal(1);
                end

                $display("PASS: tb_fpga_wrapper_yuv422_smoke exp=%0d act=%0d frames=%0d",
