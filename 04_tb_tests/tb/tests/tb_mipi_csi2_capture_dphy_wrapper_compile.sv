...
module tb_mipi_csi2_capture_dphy_wrapper_compile;
    ...
    initial begin
        ...
        timeout_count = 0;
        while (!cfg_init_done_o && timeout_count < 80) begin
            timeout_count = timeout_count + 1;
            @(posedge clk_sys);
        end
        if (!cfg_init_done_o) begin
            fail("internal APB boot did not complete");
        end
        if (!dphy_lp_mode_o || dphy_hs_mode_o) begin
            fail("LP idle should be visible through D-PHY wrapper debug outputs");
        end
        ...
        if (!dphy_hs_mode_o || dphy_lp_mode_o) begin
            fail("HS activity should be visible through D-PHY wrapper debug outputs");
        end
        if (dphy_lane_active_hs_o != 4'b0011 || dphy_lane_valid_hs_o != 4'b0011) begin
            fail("D-PHY wrapper should expose active and valid masks for configured lanes");
        end
        if (dphy_lane_sync_hs_o != 4'b0011 || !dphy_err_sot_hs_o) begin
            fail("D-PHY wrapper should expose sync and error debug outputs");
        end
        ...
        #1;
        if (!dphy_lp_mode_o || dphy_hs_mode_o) begin
            fail("D-PHY wrapper should return to LP idle");
        end

        $display("PASS: tb_mipi_csi2_capture_dphy_wrapper_compile");
        $finish;
    end
